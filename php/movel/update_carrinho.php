...
// verifica se todos os campos necessários foram enviados ao servidor
    if (isset($_POST['codigo_produto']) && isset($_POST['email']) && isset($_POST['quantidade'])) {
        ...
        $consulta_codigo_cliente->bindParam(':email', $email);
        if ($consulta_codigo_cliente->execute()) {
            if ($consulta_codigo_cliente->rowCount() > 0) {
                ...
                $consulta_deletar_item_carrinho = $db_con->prepare("UPDATE item_carrinho SET quantidade = :novaQuantidade WHERE fk_PRODUTO_codigo = :codigo_produto AND fk_CLIENTE_FK_USUARIO_codigo = :codigo_cliente");
                
                $consulta_deletar_item_carrinho->bindParam(':novaQuantidade', $quantidade);
                ...
                if ($consulta_deletar_item_carrinho->execute()) { 
                    ...
                    $resposta["sucesso"] = 0;
                    $resposta["mensagem"] = "Erro ao atualizar quantidade do item.";
                }
            } else {
