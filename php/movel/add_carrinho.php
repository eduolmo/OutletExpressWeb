	...
	if (isset($_POST['codigo_produto']) && isset($_POST['email']) && isset($_POST['quantidade'])) {
		...
		if($consulta_codigo_cliente->execute()){
			...
			if ($row && isset($row['codigo'])) {
				...
				$consulta_item_carrinho_existe = $db_con->prepare("SELECT * FROM item_carrinho WHERE fk_cliente_fk_usuario_codigo= '$codigo_cliente' AND fk_produto_codigo= '$codigo_produto'");
