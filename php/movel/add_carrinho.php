	...
	if (isset($_POST['codigo_produto']) && isset($_POST['email']) && isset($_POST['quantidade'])) {
		...
		if ($consulta_usuario_existe->rowCount() > 0) { 
			...
			if($consulta_codigo_cliente->execute()){
				$consulta_item_carrinho_existe = $db_con->prepare("SELECT * FROM item_carrinho WHERE fk_cliente_fk_usuario_codigo= '$consulta_codigo_cliente' AND fk_produto_codigo= '$codigo_produto'");
				$consulta_item_carrinho_existe->execute();
				if ($consulta_item_carrinho_existe->rowCount() > 0) { 
				...
				// Caso ocorra um erro na execução da consulta
				$resposta["sucesso"] = 0;
				$resposta["mensagem"] = "Erro na consulta do codigo do cliente.";
			}
		}
		else {
