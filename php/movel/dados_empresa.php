...
/*
 * O codigo seguinte retorna os atributos da empresa
...
if(autenticar($db_con)) {
	...
	if (isset($_GET["codigo"])) {
		...
		$consulta = $db_con->prepare("SELECT * FROM EMPRESA WHERE FK_USUARIO_codigo = $codigo");
		...
		if ($consulta->execute()) {
			if ($consulta->rowCount() > 0) {
				...
				$resposta["cnpj"] = $linha["cnpj"];
				$resposta["link"] = $linha["link"];		
				...
				// motivo da falha.
				$resposta["sucesso"] = 0;
				$resposta["erro"] = "Empresa não encontrada
                ";
			}
		} else {
